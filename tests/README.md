...
Core tests take longer than any other jyocoin tests, due to the high amount of computational work involved in validating core components.
...
To run only jyocoin's core tests (after building):
...
To run only jyocoin's crypto tests (after building):
...
```
jyocoind --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `jyocoin-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only jyocoin's hash tests (after building):
...
To run only jyocoin's performance tests (after building):
...
To run only jyocoin's unit tests (after building):
