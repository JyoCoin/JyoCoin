...
*Setup instructions for a Gitian build of jyocoin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the jyocoin CLI
...
```bash
cp jyocoin/contrib/gitian/gitian-build.py .
```
...
```
git clone git@github.com:jyocoin-project/gitian.sigs.git
...
gpg --output $VERSION-linux/$NAME/jyocoin-linux-$VERSION-build.assert.sig --detach-sign $VERSION-linux/$NAME/jyocoin-linux-$VERSION-build.assert
gpg --output $VERSION-osx-unsigned/$NAME/jyocoin-osx-$VERSION-build.assert.sig --detach-sign $VERSION-osx-unsigned/$NAME/jyocoin-osx-$VERSION-build.assert
gpg --output $VERSION-win-unsigned/$NAME/jyocoin-win-$VERSION-build.assert.sig --detach-sign $VERSION-win-unsigned/$NAME/jyocoin-win-$VERSION-build.assert
```

Make a pull request (both the `.assert` and `.assert.sig` files) to the
[jyocoin-project/gitian.sigs](https://github.com/jyocoin-project/gitian.sigs/) repository:
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/jyocoin-linux-*-build.assert
    gpg --detach-sign ${VERSION}-win-unsigned/${SIGNER}/jyocoin-win-*-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/jyocoin-osx-*-build.assert
```
